...
class WriteContext:
    def __init__(self, fname, content, msg, mode="w"):
        ...
        self.msg = msg
    def __enter__(self):
        self.file_ = open(str(self.fname), self.mode, encoding="utf-8")
...
def Read(file_name: Union[str, Path], msg="Error"):
    ...
    with ReadContext(file_name, msg) as file_:
        content = file_.read()
    return content
def ReadBytes(file_name: Union[str, Path], msg="Error"):
    if isinstance(file_name, str):
        file_name = Path(file_name)
    content = ""
    with ReadContext(file_name, msg) as file_:
...
def WriteBytesAdd(file_name: str, content_bytes: bytes):
    with open(file_name, "ab") as f:
        f.write(content_bytes)
def ReadBytes(file_name: str) -> bytes:
...
def line_continues_fn(skipped=0):
    ...
    return cont_fn
def make_it_summary(
    content: str, max_line_num: int = 10, add_continue: bool = False
) -> tuple:
    content_tup: tuple = tuple(f"{x}" for x in content.splitlines())
    new_content_tup: tuple = content_tup[0:max_line_num]
    skipped_lines = (
        len(content_tup) - max_line_num if len(content_tup) - max_line_num > 0 else 0
    )
...
def file_exists_show_and_return(file_name: Union[str, Path]) -> bool:
    ...
    if file_name.is_file():
        ...
        cont_msg, skipped = make_it_summary(
            make_indented(content_), add_continue=False, max_line_num=10
        )
        print(cont_msg)
